...
Run the `ssh-keygen` command to create an RSA-based SSH key-pair. Name the key `azure_key`. Be sure to place the key pair in the keys directory. Verify that it is there.
...
3. Login using the Azure CLI
    ...
    > Note: If it fails, you may need to augment the command:
    > `az login --tenant <TENANT_ID>`.
    > Type in the Tenant ID (likely listed in the CLI) and if necessary, then authenticate with MFA via your smartphone or other device.
    >
    > If you are still having issues, try making sure that you are logged in to the Azure website within the correct browser.
...
As of version 4 of the Azure provider plugin, you need to enter the Microsoft Azure Subscription ID into the azure provider block. 

Locate the provider block in `azure.tf`. It should look like this:

```hcl
provider "azurerm" {
  features {}
  subscription_id = "<YOUR_SUBSCRIPTION_ID>"  # Now required in v4.0
}
```

Go to https://portal.azure.com/#home, then Subscriptions, and copy in your subscription ID into the double-quotes within the `subscription_id` argument. 
...
This can take a couple minutes. You might also see a delay after you execute `terraform apply` where it seems like nothing is happening. BE PATIENT! Continue analyzing the azure.tf code (but don't make any changes!) while the terraform run completes.

When finished, check the infrastructure at the Azure web console. 

https://portal.azure.com

The virtual machine should be listed right in the Home page. To learn more about it just click it's link. 

To see additional resources that were created, click `see all` underneath the virtual machine. (This will take you to the Resource Manager.) That should show the network infrastructure, VM, NIC, and everything else needed to function. 
...
`ssh -i "keys/azure_key" adminuser@<ip_address>`
