# ⚙️ Lab 06 - Terraform with cloud-init and Viewing Resources
...
- View the hello.txt file in the home directory, and the new website located at /var/www/html
...
- Attempt to connect to the website from a browser at: 
  ...
  > Tip: Automatically open the browser to the IP address of the instance from the VS Code terminal. Example: `firefox <ipaddress>`. Replace the browser with your tool of choice!

- Play around with it! You did good!

  > Note: to see a *real* example of mkdocs in action, check out the Draw On Gnome extension documentation: 

  > https://daveprowse.github.io/Draw-On-Gnome/
...
There is another option for specific changes to a resource and that is the `-target` option.
...
Or, target an individual module's resource. Example:
	
```
terraform apply -target=module.<module_name>.<resource_name>
```

> Note: This can cause issues, as resource targeting is in effect and is not for routine use.
